    ...
    if (!isStarted) {
        return (
            <div className="max-w-2xl mx-auto py-16 px-4 text-center">
                <div className="bg-white p-8 rounded-2xl shadow-xl border border-brand-100">
                    <div className="w-20 h-20 bg-brand-100 rounded-full flex items-center justify-center mx-auto mb-6">
                        <Mic2 className="w-10 h-10 text-brand-600" />
                    </div>
                    ...
                    <input
                        className="w-full p-4 border border-gray-300 rounded-xl text-lg mb-6 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 text-center"
            ...
            <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    {messages.map((msg, idx) => (
                        <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[85%] p-5 rounded-2xl text-sm leading-relaxed shadow-sm ${msg.role === 'user' ? 'bg-brand-600 text-white rounded-br-none' : 'bg-gray-50 text-gray-800 border border-gray-100 rounded-tl-none'}`}>
                ...
                <div className="p-4 border-t border-gray-100 bg-gray-50/50">
                    <div className="flex gap-3">
                        <textarea
                            className="flex-1 p-4 border border-gray-200 rounded-xl resize-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none shadow-sm bg-white"
                            rows={2}
                            placeholder="输入你的回答..."
                            value={input}
