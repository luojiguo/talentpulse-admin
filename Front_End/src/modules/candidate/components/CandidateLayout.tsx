...
const CandidateLayout: React.FC<CandidateLayoutProps> = ({ children, currentUser, onLogout, onSwitchRole }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFB] dark:bg-slate-900 flex flex-col font-sans transition-colors duration-300">
      <header className="bg-white/70 dark:bg-[#1C1C1E]/70 backdrop-blur-xl border-b border-gray-200/50 dark:border-white/10 sticky top-0 z-40 shrink-0 transition-all duration-300 supports-[backdrop-filter]:bg-white/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center gap-1 flex-1 justify-center">
            {[
              { path: '/', label: t.candidate.nav.home },
              { path: '/job', label: t.candidate.nav.jobs },
              { path: '/saved', label: t.candidate.nav.saved },
              { path: '/interviews', label: t.candidate.nav.interviews },
              { path: '/messages', label: t.candidate.nav.messages },
              { path: '/mock-interview', label: t.candidate.nav.mockInterview },
              { path: '/ai-chat', label: t.candidate.nav.aiAssistant }
            ].map((item) => (
              <button
                key={item.path}
                onClick={() => navigate(item.path)}
                className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 relative ${isActive(item.path)
                  ? 'text-white'
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                ...
                <UserAvatar
                  ...
                  size={36}
                  className="bg-brand-100 text-brand-700 border-2 border-brand-200"
                  alt="头像"
                />
                <svg
        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-white dark:bg-slate-800 border-t border-brand-100 dark:border-slate-700 shadow-lg">
            <div className="px-4 py-3 space-y-1">
              ...
              >
                {t.candidate.nav.home}
              </button>
              <button
                onClick={() => { navigate('/job'); setMobileMenuOpen(false); }}
                className={`block w-full text-left px-3 py-2 rounded-md text-base font-medium transition-colors ${isActive('/job')
                  ? 'text-white bg-brand-500 dark:bg-brand-500'
                  : 'text-slate-600 dark:text-slate-300 hover:bg-brand-50 dark:hover:bg-slate-700 hover:text-brand-600 dark:hover:text-brand-400'
                  }`}
              >
                {t.candidate.nav.jobs}
              </button>
              <button
                onClick={() => { navigate('/saved'); setMobileMenuOpen(false); }}
                className={`block w-full text-left px-3 py-2 rounded-md text-base font-medium transition-colors ${isActive('/saved')
                  ? 'text-white bg-brand-500 dark:bg-brand-500'
                  : 'text-slate-600 dark:text-slate-300 hover:bg-brand-50 dark:hover:bg-slate-700 hover:text-brand-600 dark:hover:text-brand-400'
                  }`}
              >
                {t.candidate.nav.saved}
              </button>
              <button
                onClick={() => { navigate('/interviews'); setMobileMenuOpen(false); }}
                className={`block w-full text-left px-3 py-2 rounded-md text-base font-medium transition-colors ${isActive('/interviews')
                  ? 'text-white bg-brand-500 dark:bg-brand-500'
                  : 'text-slate-600 dark:text-slate-300 hover:bg-brand-50 dark:hover:bg-slate-700 hover:text-brand-600 dark:hover:text-brand-400'
                  }`}
              >
                {t.candidate.nav.interviews}
              </button>
              <button
                onClick={() => { navigate('/messages'); setMobileMenuOpen(false); }}
                className={`block w-full text-left px-3 py-2 rounded-md text-base font-medium transition-colors ${isActive('/messages')
                  ? 'text-white bg-brand-500 dark:bg-brand-500'
                  : 'text-slate-600 dark:text-slate-300 hover:bg-brand-50 dark:hover:bg-slate-700 hover:text-brand-600 dark:hover:text-brand-400'
                  }`}
              >
                {t.candidate.nav.messages}
              </button>
              <button
                onClick={() => { navigate('/mock-interview'); setMobileMenuOpen(false); }}
                className={`block w-full text-left px-3 py-2 rounded-md text-base font-medium transition-colors ${isActive('/mock-interview')
                  ? 'text-white bg-brand-500 dark:bg-brand-500'
                  : 'text-slate-600 dark:text-slate-300 hover:bg-brand-50 dark:hover:bg-slate-700 hover:text-brand-600 dark:hover:text-brand-400'
                  }`}
              >
                {t.candidate.nav.mockInterview}
              </button>
              <button
                onClick={() => { navigate('/ai-chat'); setMobileMenuOpen(false); }}
                className={`block w-full text-left px-3 py-2 rounded-md text-base font-medium transition-colors ${isActive('/ai-chat')
                  ? 'text-white bg-brand-500 dark:bg-brand-500'
                  : 'text-slate-600 dark:text-slate-300 hover:bg-brand-50 dark:hover:bg-slate-700 hover:text-brand-600 dark:hover:text-brand-400'
                  }`}
              >
                {t.candidate.nav.aiAssistant}
              </button>
            </div>
          </div>
        )}
