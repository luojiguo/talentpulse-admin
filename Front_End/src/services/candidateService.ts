...
export const candidateAPI = {
  // 获取所有求职者
  getAllCandidates: () => request.get('/api/candidates'),

  // 获取求职者详情
  getCandidateProfile: (userId: string | number) => request.get(`/api/candidates/${userId}`),

  // 更新/创建求职者详情
  updateCandidateProfile: (userId: string | number, data: any) => {
    return request.post(`/api/candidates/${userId}`, data);
  },
  ...
  getCandidateSavedJobs: (userId: string | number) => {
    return request.get(`/api/candidates/${userId}/saved-jobs`);
  },

  // 保存职位到收藏
  saveJob: (userId: string | number, jobId: string | number) => {
    return request.post(`/api/candidates/${userId}/saved-jobs`, { jobId });
  },

  // 从收藏中移除职位
  removeSavedJob: (userId: string | number, jobId: string | number) => {
    return request.delete(`/api/candidates/${userId}/saved-jobs/${jobId}`);
  },

  // 获取求职者的收藏公司
  getCandidateSavedCompanies: (userId: string | number) => {
    return request.get(`/api/candidates/${userId}/saved-companies`);
  },

  // 保存公司到收藏
  saveCompany: (userId: string | number, companyId: string | number) => {
    return request.post(`/api/candidates/${userId}/saved-companies`, { companyId });
  },

  // 从收藏中移除公司
  removeSavedCompany: (userId: string | number, companyId: string | number) => {
    return request.delete(`/api/candidates/${userId}/saved-companies/${companyId}`);
  },
