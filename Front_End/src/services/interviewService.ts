...
export const interviewAPI = {
  ...
  getInterviewsByApplicationId: (applicationId: string | number) => {
    return request.get(`/api/interviews/application/${applicationId}`);
  },

  // 创建面试
  createInterview: (data: any) => {
    return request.post('/api/interviews', data);
  },

  // 更新面试
  updateInterview: (id: string | number, data: any) => {
    return request.patch(`/api/interviews/${id}`, data);
  },

  // 更新面试状态
  updateInterviewStatus: (id: string | number, status: string) => {
    return request.patch(`/api/interviews/${id}/status`, { status });
  },

  // 删除面试
  deleteInterview: (id: string | number) => {
    return request.delete(`/api/interviews/${id}`);
  },
};
